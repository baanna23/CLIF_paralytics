...
# Helper function to standardize Arrow string types when loading datasets
# Converts large_string and large_utf8 to standard utf8 and handles null types
standardize_arrow_types <- function(filepath) {
  ds <- open_dataset(filepath)
  schema_info <- schema(ds)

  # Identify columns with large_string or large_utf8 types
  large_string_cols <- names(schema_info)[sapply(schema_info, function(field) {
    field$type$name %in% c("large_utf8", "large_string")
  })]

  # Identify columns with null type (problematic for joins)
  null_cols <- names(schema_info)[sapply(schema_info, function(field) {
    field$type$name == "null"
  })]

  # Cast large string columns to standard utf8 if any exist
  if (length(large_string_cols) > 0) {
    ds <- ds |> mutate(across(all_of(large_string_cols), ~cast(., utf8())))
  }

  # Drop null-type columns as they can't be used in joins
  if (length(null_cols) > 0) {
    ds <- ds |> select(-all_of(null_cols))
  }
...
if (file_type == "parquet") {
  # Use standardize_arrow_types to handle large_string/large_utf8 conversions
  data_list <- lapply(required_filenames, standardize_arrow_types)
