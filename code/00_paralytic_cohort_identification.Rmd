...
para_24<- time_to_paralytic_tmp_i_rose |>
 ...
 # filter(intermittent_para24>2) |>
  select(hospitalization_id, intermittent_para24)
para_24 <- unique(para_24)

para_48<- time_to_paralytic_tmp_i_rose |>
  filter(criteria_to_this_paralytic_i<48 & criteria_to_this_paralytic_i>0) |> #must be in first 24h after criteria
...
para_72<- time_to_paralytic_tmp_i_rose |>
  filter(criteria_to_this_paralytic_i<72 & criteria_to_this_paralytic_i>0) |> #must be in first 24h after criteria
  mutate(intermittent_para72=sum(how_many))|>
 # filter(intermittent_para72>2) |>
...
para_0<- time_to_paralytic_tmp_i_rose |>
 ...
 # filter(intermittent_para0>2) |>
  select(hospitalization_id, intermittent_para0)
para_0 <- unique(para_0)

para_12<- time_to_paralytic_tmp_i_rose |>
  filter(criteria_to_this_paralytic_i<12 & criteria_to_this_paralytic_i>0) |> #must be in first 24h after criteria
...
final_cohort_rose_rose<- rose_criteria_timing |>
  ...
  filter(is.na(criteria_to_paralytic_c) | criteria_to_paralytic_c>-3)  |> #can't start prior to 3 hours before meeting criteira *** need to make to time <3h prior to criteria. *** 
...
cat('This yields an eligible ROSE cohort of', length(unique(final_cohort_rose_rose$hospitalization_id)), 'hospitalizations, of which',
    ...
    sum(final_cohort_rose_rose$any_paralytic_72), 'were started on paralytics in 72 hours, all defined by continuous dosing or at least 3 pushes.',    
    sum(final_cohort_rose_rose$any_paralytic_ever), 'were ever started on paralytics, all defined by continuous dosing or at least 1 push \n') 
...
max_rass <- rass |>
 filter(as.POSIXct(recorded_dttm, tz="UTC")>=as.POSIXct(vent_episode_start, tz="UTC") &
           ...
           as.POSIXct(recorded_dttm)<=as.POSIXct(t_rose_first)) |>
  group_by(patient_id, encounter_block) |>
  mutate(max_rass=suppressWarnings(max(numerical_value, na.rm = TRUE))) |>
  filter(row_number()==1) |>
  select(patient_id, encounter_block, max_rass) |> 
  ungroup()
