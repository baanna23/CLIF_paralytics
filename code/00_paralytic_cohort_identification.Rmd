...
encounters <- length(unique(clif_hospitalization$encounter_block))
...
encounters <- length(unique(vent$encounter_block))
...
vent <- vent |>
  ...
  arrange(encounter_block, recorded_dttm) |>
...
next_exclusion <- data.frame(
  encounters = length(unique(vent_eligible$encounter_block)),
...
trach <- clif_respiratory_support |>
  ...
  inner_join(cohort_ids, by=c('hospitalization_id')) |>
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_eligible$encounter_block[vent_eligible$trach_within_24==1])),
...
vent_eligible <- vent_eligible |>
  ...
  mutate(transfer_on_vent = coalesce(transfer_on_vent, 0))
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_eligible$encounter_block[vent_eligible$transfer_on_vent==1])),
...
pao2 <- clif_labs |>
    ...
    lab_value_numeric > 40,
    lab_value_numeric <= 700
  ) |>
   transmute(patient_id,
    hospitalization_id,
    encounter_block,
    lab_collect_dttm,
    pao2 = lab_value_numeric
  ) |>
  distinct() |>
  collect() |>
...
vent_data <- clif_respiratory_support |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  select(-any_of(c("mode_name", "lpm_set", "pressure_control_set", "pressure_support_set", "flow_rate_set", "inspiratory_time_set", "mean_airway_pressure_obs"))) |>
  inner_join(cohort_ids) |>
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_eligible$encounter_block)) - length(unique(rose_windowed$encounter_block)),
    involved_patients = length(unique(vent_eligible$patient_id)) - length(unique(rose_windowed$patient_id)),
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(rose_criteria_timing$encounter_block)) - length(unique(inclusions$encounter_block)),
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(final_cohort_rose_rose$encounter_block)) - length(unique(inclusions$encounter_block)),
...
hospitalization_vars <- clif_hospitalization |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(
    cohort_ids |> select(patient_id, encounter_block) |> distinct(),
...
code_status <- clif_code_status |>
  ...
  select(patient_id, encounter_block, start_dttm, code_status_category, code_status_name, t_rose_first, paralytic_time) |>
  ungroup()
...
code_status_rose <- code_status |>
  ...
  slice_max(start_dttm, n=1, with_ties = F) |> #if 2 code status entered at exact same time, randomly select 1
...
paralytic_start_time <- cohort_times |>
  select(patient_id, encounter_block, paralytic_time)
...
sbp <- clif_vitals |>
  ...
  select(-vital_name, -vital_category, -any_of("meas_site_name")) |>
  filter(sbp >= 0 & sbp<250) |>
  collect()

sbp_clean <- sbp %>%
  group_by(recorded_dttm, patient_id, encounter_block) %>%
