...
# Helper to remove objects only if they exist, then run garbage collection
cleanup <- function(...) {
  objs <- as.character(substitute(list(...)))[-1]
  objs <- objs[objs %in% ls(envir = parent.frame())]
  if (length(objs) > 0) {
...
tables <- c("patient", "hospitalization", "vitals", "labs", 
            ...
            "admission_diagnosis", "provider", "sensitivity", "hospital_diagnosis",
...
true_tables <- c("patient", "hospitalization", "adt", "patient_assessments", "hospital_diagnosis",
...
#Create Hospital Blocks Here (Some Transfers Will begin with ED at One Hospital and Continue in ICU/Ward/Stepdown at the Next)
# Collect as data table
hospitalizations <- as.data.table(clif_hospitalization)
...
hospitalizations <- hospitalizations[, `:=`(
  ...
  final_discharge_category = discharge_category[which.max(end_ts)],
  # Stores the discharge location from last admission within block
  final_discharge_name = discharge_name[which.max(end_ts)]
), by = .(patient_id, block_num)]


# Create unique hospital block IDs
hospitalizations[, encounter_block := paste0(patient_id, "_", block_num)]

#Extract Unique IDs
encounter_blocks = select(hospitalizations, c('patient_id', 'hospitalization_id', 'encounter_block')) |> as_arrow_table()

# Create One Row per Hospitalization Block and Extract Info For Consort
clif_hospitalization <- hospitalizations[
  order(encounter_block, admission_dttm), 
  .SD[1], 
  by = encounter_block
] |>
  as_arrow_table() |>
...
inpatient_hospitalization_ids <- clif_adt |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(encounter_blocks %>% select(hospitalization_id, encounter_block), 
...
clif_hospitalization <- clif_hospitalization |>
  semi_join(inpatient_hospitalization_ids, by='encounter_block') |>
  compute()
cleanup(inpatient_hospitalization_ids)

#Update Encounter Blocks with Eligible Encounters
encounter_blocks <- encounter_blocks |>
  semi_join(clif_hospitalization ,by='encounter_block') |>
  compute()
...
consort <- data.frame(
  encounters = encounters,
    patients = patients,
  reason = "total in database"
)

cleanup(patients, encounters)
```
...
ever_vent <- clif_respiratory_support |>
  filter(tolower(device_category)=='imv') |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  #Only Need IDs in the Current Working clif hospitalization Table
  #This is the Joint Table with Those Who Ever Received Mechanical Ventilation
  semi_join(encounter_blocks, 
             by='hospitalization_id') |>
...
vent <- clif_respiratory_support |> 
    mutate(hospitalization_id=as.character(hospitalization_id)) |>
    semi_join(ever_vent, by='hospitalization_id') |>
    inner_join(encounter_blocks, by='hospitalization_id') |>
   compute()
...
next_exclusion <- data.frame(
  encounters = encounters,
    patients = patients,
  reason = "initial number on IMV before cleaning"
)
...
next_exclusion <- data.frame(
  encounters = dim(vent_eligible)[1],
    patients = length(unique(vent_eligible$patient_id)),
  reason = "actual initial number on IMV after ensuring data in IMV episode"
)

consort <- bind_rows(consort, next_exclusion)

cleanup(device_transitions, vent, vent_episodes, patients, encounters)
...
cohort_ids <- encounter_blocks |>
  inner_join(temp_ids, by=c('patient_id', 'encounter_block')) |>
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_eligible$hospitalization_id[vent_eligible$trach_within_24==1])),
    involved_patients = length(unique(vent_eligible$patient_id[vent_eligible$trach_within_24==1])),
  reason = "trach within 24h"
)
...
osh_transfer <- clif_adt |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(cohort_ids) |> #Here need to join first and then filter to those in cohort
...
cleanup(osh_transfer, first_device)
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_eligible$hospitalization_id[vent_eligible$transfer_on_vent==1])),
    involved_patients = length(unique(vent_eligible$patient_id[vent_eligible$transfer_on_vent==1])),
  reason = "transfer OSH on vent"
)
...
clif_labs <- clif_labs |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(cohort_ids) |> #NOTE: clif_labs will now include patient_id and encounter_block
...
clif_vitals <- clif_vitals |>
  mutate(hospitalization_id = as.character(hospitalization_id)) |>
  inner_join(cohort_ids) |> #NOTE: clif_vitals will now include patient_id and encounter_block
  select(-in_cohort) |>
  compute()
...
next_exclusion <- data.frame(
  excluded_encounters = length(unique(vent_data$hospitalization_id)) - length(unique(rose_windowed$hospitalization_id)),
    involved_patients = length(unique(vent_data$patient_id)) - length(unique(rose_windowed$patient_id)),
  reason = "don't have p/f or s/f data"
)
...
cleanup(rose_windowed, rose_criteria, qual_vars, find_first_qualifying)
...
cleanup(paralytics_episodes_all_c)
...
cleanup(time_to_paralytic_tmp_c)
...
cleanup(paralytics_episodes_all_i)
...
cleanup(time_to_paralytic_tmp_i, first_continuous_start)
...
cleanup(
  ...
  para_12,
  para_ever,
  paralytics_episodes_c,
  paralytics_episodes_i,
  temp_times
)
...
inclusions <- rose_criteria_timing |>
  filter(is.na(criteria)==F)

next_exclusion <- data.frame(
  excluded_encounters = length(unique(rose_criteria_timing$hospitalization_id)) - length(unique(inclusions$hospitalization_id)),
    involved_patients = length(unique(rose_criteria_timing$patient_id)) - length(unique(inclusions$patient_id)),
  reason = "don't meet ROSE criteria"
)
...
inclusions <- final_cohort_rose_rose |>
  filter(total_continuous_para_hours_prior < 3)

next_exclusion <- data.frame(
  excluded_encounters = length(unique(final_cohort_rose_rose$hospitalization_id)) - length(unique(inclusions$hospitalization_id)),
    involved_patients = length(unique(final_cohort_rose_rose$patient_id)) - length(unique(inclusions$patient_id)),
  reason = "3+ hours continuous paralytics before criteria"
)

consort <- bind_rows(consort, next_exclusion)


final_cohort_rose_rose <- final_cohort_rose_rose |>
...
inclusions <- final_cohort_rose_rose |>
  filter(is.na(criteria_to_paralytic_i_rose) | intermittent_para0 <3)

next_exclusion <- data.frame(
  excluded_encounters = length(unique(final_cohort_rose_rose$hospitalization_id)) - length(unique(inclusions$hospitalization_id)),
    involved_patients = length(unique(final_cohort_rose_rose$patient_id)) - length(unique(inclusions$patient_id)),
  reason = "3+ intermittent pushes prior to starting"
)
...
#Update Running Table of Cohort IDs
#List of patient_id, encounter_block, hospitalization_id and vent_start
temp_ids <- final_cohort_rose_rose |>
...
cleanup(
  temp_ids,
  paralytic_start_i,
  paralytic_start_i_rose,
  paralytic_start_c,
  paralytic_start_all_rose,
  paralytics_prior_duration,
  paralytics_post_duration,
  paralytics_total_duration,
  paralytics_48h_duration,
  time_to_paralytic_c,
  time_to_paralytic_i,
  time_to_paralytic_i_rose
)
```
...
cleanup(temp_times, prone_episodes_all, position_output)
```
...
#ICD code variable
diagnosis <- clif_hospital_diagnosis |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(cohort_ids) |>
  collect() |>
  mutate(
  ...
  group_by(hospitalization_id, patient_id, encounter_block) |>
  mutate(
    #1st Definition ARDS Code, doesn't matter if i50 CHF code
    ards_icd_10 = fifelse(sum(ards_code, na.rm=T) >= 1, 1, 0, 0
    ))|>
  ffirst() |>
...
hospital_location <- clif_adt |>
  mutate(hospitalization_id=as.character(hospitalization_id)) |>
  inner_join(cohort_hospitalization_ids) |>
...
cleanup(pressor_data, pressor_units)
...
cleanup(sbp, sbp_clean, dbp, dbp_clean, map, map_clean, bp)
...
cleanup(
  hospitalization_vars,
  demos,
  hospital_location,
  bmi,
  final_cohort_rose_rose,
  vent_times,
  prone_episodes,
  min_ph,
  code_status_rose,
  ...
  sars_cov2,
  paralytic_start_all,
  code_status_paralytics,
  set_tv_paralytic,
  set_rr_paralytic,
  mv_obs_paralytic,
  max_rass12_paralytics,
  min_rass12_paralytics,
  min_ph_paralytics,
  paralytic_start_time,
  cohort_times
)
...
hospitalization_ids <- unique(hospitalization_ids)
...
cleanup(
  ...
  clif_position,
  clif_microbiology_nonculture,
  clif_code_status, 
  hospitalization_ids, cohort_hospitalization_ids, ahrf_cohort, clif_hospital_diagnosis, code_status, diagnosis, rose_criteria_timing, vent_data, vent_eligible, year, trach, plat_tmp, next_exclusion, inclusions, cohort_ids, rose_analytic_data)
